...
void ServerLobby::submitRankingsToAddons()
{
    ...
    {
    public:
        SubmitRankingRequest(uint32_t online_id, double raw_scores,
                             double scores, double max_scores,
                             unsigned num_races, double rating_deviation,
    ...
    for (unsigned i = 0; i < RaceManager::get()->getNumPlayers(); i++)
    {
        ...
        auto request = std::make_shared<SubmitRankingRequest>
            (id, m_raw_scores.at(id), m_scores.at(id),
            m_max_scores.at(id), m_num_ranked_races.at(id), 
